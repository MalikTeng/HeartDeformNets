...
def mesh_loss_geometric_cf(feed_dict, weights, mesh_id, cf_ratio=1., mesh_weights=[0.5, 0.5, 0.5], if_mask=True, if_l2=True, if_cap=False):
    def loss(y_true, y_pred):
        losses = mesh_loss(y_pred, y_true, feed_dict, mesh_id, mesh_weights, cf_ratio, if_mask, if_l2, if_cap)
...
def mesh_loss_geometric_cf_ffd(feed_dict, weights, mesh_id, cf_ratio=1., if_mask=True, if_l2=True, if_cap=False):
    def loss(y_true, y_pred):
        ctr_list = feed_dict['ctr_data'][mesh_id]
        side_list = feed_dict['side_data'][mesh_id]
        cap_id_data = feed_dict['cap_data'][mesh_id]
        point_loss, normal_loss, edge_loss, lap_loss, cap_loss = mesh_loss_single(y_pred, y_true, feed_dict['tmplt_faces'][mesh_id], None, ctr_list, side_list, cap_id_data, cf_ratio, if_mask, mesh_id=mesh_id, if_cap=if_cap)
        total_loss = tf.pow(point_loss*10, weights[0])*tf.pow(normal_loss*10, weights[1]) + 20.*cap_loss
        return total_loss
    return loss
...
def mesh_loss_single(pred, gt, tmplt_faces, lap_ids, ctr_list, side_list, cap_id_data, cf_ratio=1., if_mask=False, mesh_id=0, if_cap=False):
    ...
    # if adding mask for shape completion
    if if_mask:
...
def mesh_loss(pred, gt, feed_dict, mesh_id, weights, cf_ratio=1., if_mask=False, if_l2=True, if_cap=False):
    gt_pt = gt[:, :, :3] # gt points
    gt_nm = gt[:, :, 3:] # gt normals
