...
def sparse_tensor_dense_tensordot(sp_a, b, axes, name=None):
    ...
    def _tensordot_reshape(a, axes, flipped=False):
        ...
        if a.get_shape().is_fully_defined() and isinstance(axes, (list, tuple)):
            ...
            rank_a = tf.rank(a)
            if TF2:
                axes = tf.convert_to_tensor(axes, dtype=tf.int32)
            else:
    ...
    if TF2:
        ...
        if isinstance(sp_a_free_dims, list) and isinstance(b_free_dims, list):
            #return tf.reshape(ab_matmul, sp_a_free_dims + b_free_dims, name=scope)
            ...
            #product = tf.reshape(
            #        ab_matmul, tf.concat([sp_a_free_dims, b_free_dims], 0), name=scope)
            product = tf.reshape(
                    ab_matmul, tf.concat([sp_a_free_dims, b_free_dims], 0))
